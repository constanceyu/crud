...
-- you can use OLD and NEW to update values
-- for INSERT there is no OLD, only NEW
CREATE TRIGGER ttl_val_ins AFTER INSERT ON object
    FOR EACH ROW
    UPDATE collection
    SET total_value = total_value + NEW.value
    WHERE collection.id=NEW.collectionid;
    ...
    FOR EACH ROW
        BEGIN
            ...
            IF OLD.collectionid <> NEW.collectionid THEN
                ...
                WHERE collection.id=OLD.collectionid;
                UPDATE collection
                SET total_value = total_value + NEW.value
                WHERE collection.id = NEW.collectionid;
            ELSE
                ...
                -- NEW.collectionid could also be substituted by 
                -- OLD.collectionid or just collectionid
